document.addEventListener("DOMContentLoaded", () => {
  ...
  /*
  // Show/hide auth-only nav items based on login state
  function updateNavAuthVisibility() {
    const user = localStorage.getItem("pillpallUser");
    const authHrefs = [
      "medicine.html",
      "timetable.html",
      "profile.html",
    ...
    document.querySelectorAll(".nav-links a").forEach((link) => {
      ...
      if (authHrefs.includes(href)) {
        link.style.display = user ? "" : "none";
      }
    });

    // Ensure login button reflects state (keeps existing behavior but centralised here)
  ...
  */
  ...
  if (addBtn) {
    ...
    let medicines = JSON.parse(
      localStorage.getItem("pillpallMedicines") || "[]"
    );
  ...
  if (weekView && viewToggle) {
    ...
    const timetableData = JSON.parse(
      localStorage.getItem("pillpallTimetable") || "{}"
    );

    const saveData = () =>
      localStorage.setItem("pillpallTimetable", JSON.stringify(timetableData));
    ...
    const renderCell = (cell, key) => {
      ...
      tagContainer.addEventListener("click", (e) => {
        if (e.target.tagName === "BUTTON") {
          const medName = e.target.parentElement.textContent
            .trim()
            .slice(0, -1);
    ...
    viewToggle.addEventListener("click", () => {
      ...
      viewToggle.textContent = monthMode
        ? "Switch to Week View"
